...
def do_commit():
	...
	if localrepo==None:
		...
		deleted = []
		for path in tree:
			absfile = os.path.join(localrepo, path)
			if not os.path.exists(absfile) and showquestion("Commit", "Delete " + path + "?"):
				deleted.append(path)
		for path in deleted:
			del tree[path]
		for root, dirs, files in os.walk(localrepo):
			for f in files:
				...
				relfile = os.path.relpath(absfile, localrepo)
				newmode = os.stat(absfile).st_mode
				with open(absfile, "rb") as file:
					newblob = Blob.from_string(file.read())
					if relfile in tree:
						oldmode, oldsha = tree[relfile]
						if oldmode != newmode or oldsha != newblob.id:
							if showquestion("Commit", "Change " + relfile + "?"):
								changes = True
								object_store.add_object(newblob)
								tree[relfile] = (newmode, newblob.id)
					else:
						if showquestion("Commit", "Add " + relfile + "?"):
							changes = True
							object_store.add_object(newblob)
							tree.add(relfile, newmode, newblob.id)
			if ".git" in dirs:
				dirs.remove(".git")
			for d in dirs:
				absdir = os.path.join(root, d)
				reldir = os.path.relpath(absdir, path)
				if not showquestion("Commit", "Add directory " + reldir + "?"):
					dirs.remove(d)
		if changes:
			...
			commit.message = showinput("Message", "Enter commit message", "Initial commit")
